...
class Sprite // Inline class. Int-based destination rectangle.
{
...
protected: // Private BUT inherited.
	double m_angle;
	SDL_Rect m_src, m_dst; // Note: int-based Rects.
...
class FSprite // Inline class. Float-based destination rectangle.
{
public: // Inherited and public.
	FSprite(SDL_Rect s, SDL_FRect d, SDL_Renderer* r, SDL_Texture* t)
		:m_src(s), m_dst(d), m_pRend(r), m_pText(t), m_angle(0.0) {}
...
protected: // Private BUT inherited.
	double m_angle;
	SDL_Rect m_src;
	SDL_FRect m_dst; // Note: float-based FRect.
	SDL_Renderer* m_pRend;
	SDL_Texture* m_pText;
private: // Private NOT inherited.
...
{
public:
	AnimatedSprite(SDL_Rect s, SDL_Rect d, SDL_Renderer* r, SDL_Texture* t, int sstart, int smin, int smax, int nf)
...
class AnimatedFSprite : public FSprite// Also inline.
{
public:
	AnimatedFSprite(SDL_Rect s, SDL_FRect d, SDL_Renderer* r, SDL_Texture* t, int sstart, int smin, int smax, int nf)
		:FSprite(s, d, r, t), m_sprite(sstart), m_spriteMin(smin), m_spriteMax(smax), m_frameMax(nf) {}
	void Animate()
	{
		if (m_frame++ == m_frameMax) // Post-increment ensures m_frame starts at 0.
		{
