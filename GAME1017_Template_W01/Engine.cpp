...
void Engine::Render() 
{
	// Example-specific rendering (minus RenderPresent) should be done with a FSM and states.
	
	// Flip buffers. Keep call even with FSM.
...
void Engine::Clean()
{
	...
	// Let StateManager clean up first.
	STMA::Quit();
	// Then the rest.
	DEMA::Quit();
	EVMA::Quit();
...
int Engine::Run()
{
	if (m_running) // What does this do and what can it prevent?
		return -1;
	if (Init("GAME1017 Engine Template", SDL_WINDOWPOS_CENTERED, 
